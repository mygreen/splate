...
public class ParenBindVariableNode extends AbstractNode {
     ...
     */
    @SuppressWarnings("rawtypes")
    private void bindArray(final ListParamNodeProcessContext ctx, final Object array) {
    ...
    /**
     * 配列に変換してバインドする。
     * @param ctx the NamedParamNodeProcessContext
     * @param array the variable array
     */
    @SuppressWarnings("rawtypes")
    private void bindArray(final NamedParamNodeProcessContext ctx, final Object array) {
        int length = Array.getLength(array);
        if (length == 0) {
            return;
        }
        ...
        for (int i = 1; i < length; ++i) {
            ...
            value = Array.get(array, i);
            value = getBindVariableValue(value, valueType, getPosition(), ctx.getParsedSql(), expression);
            bindName = ctx.allocateBindName(expression);
            ctx.addSql(":" + bindName, bindName, value);
        }
        ctx.addSql(")");
